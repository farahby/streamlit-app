...
def exposure_is_true(value):
    if isinstance(value, dict):
        value = value.get("internet_facing", "UNKNOWN")
...
n_inet        = int(df["internet_facing"].map(exposure_is_true).sum())  if "internet_facing" in df.columns else 0
...
with tab8:
    ...
    if rem_df.empty:
        ...
        for _, row in rem_df.iterrows():
            ...
            blast       = (1 if row.get("in_blast_radius") == 1 else 0)
...
with tab9:
    ...
    # ── P2 : SHAP interactif avec fallback CSV ──────────────────────
    _p2 = _vload("risk_explanations.json")
    _p2_rows = (_p2 or {}).get("explanations", [])

    # Fallback : les explications sont déjà présentes dans scored_findings.csv
    if not _p2_rows and not _sdf.empty and "top_shap_drivers" in _sdf.columns:
        for _, _p2_csv_row in _sdf.iterrows():
            try:
                _p2_drivers_csv = _vjson.loads(
                    _p2_csv_row.get("top_shap_drivers") or "[]"
                )
            except Exception:
                _p2_drivers_csv = []

            _p2_rows.append({
                "finding_id": str(_p2_csv_row.get("id", "")),
                "risk_score": float(_p2_csv_row.get("risk_score", 0) or 0),
                "calibrated_probability": float(
                    _p2_csv_row.get(
                        "risk_probability_calibrated", 0
                    ) or 0
                ),
                "top_drivers": _p2_drivers_csv,
            })

    st.subheader("P2 — Explications SHAP")
    ...
    if _p2_rows:
        ...
        _p2_selected = st.selectbox(
            "Choisir un finding à expliquer",
            _p2_ids,
            key="p2_finding_selector"
        )

        _p2_selected_row = next(
            (
                x for x in _p2_rows
                if str(x.get("finding_id")) == _p2_selected
            ),
            None
        )

        if _p2_selected_row:
            _p2_c1, _p2_c2 = st.columns(2)

            with _p2_c1:
                st.metric(
                    "Risk score",
                    round(float(_p2_selected_row.get("risk_score", 0)), 2)
                )
            ...
            _p2_drivers = _p2_selected_row.get("top_drivers", [])

            if _p2_drivers:
                st.dataframe(
                    pd.DataFrame(_p2_drivers),
                    use_container_width=True,
                    hide_index=True
                )
            else:
                st.info("Aucun facteur SHAP pour ce finding.")

        st.caption(
            "Les valeurs SHAP expliquent le score ML ; "
            "elles ne modifient pas la décision du pipeline."
        )
    else:
        st.warning(
            "Aucun résultat SHAP. Vérifie que top_shap_drivers "
            "existe dans scored_findings.csv."
        )
